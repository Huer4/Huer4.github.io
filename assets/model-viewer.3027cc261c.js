...
document.querySelectorAll('[data-model-viewer]').forEach(panel=>{
const host=panel.querySelector('.geometry-viewer');
const status=panel.querySelector('[role=status]');
 ...
 new STLLoader().load(panel.dataset.modelViewer,geometry=>{
 geometry.computeBoundingBox();const size=geometry.boundingBox.getSize(new THREE.Vector3());geometry.center();geometry.scale(...Array(3).fill(2/Math.max(size.x,size.y,size.z)));geometry.computeVertexNormals();
 ...
 panel.querySelectorAll('[data-model-action]').forEach(b=>b.disabled=false);
 },undefined,()=>{status.textContent='The 3D preview could not load. Please refresh the page to try again.';});
 panel.querySelectorAll('[data-model-action]').forEach(button=>button.addEventListener('click',()=>{
 ...
 }catch(error){status.textContent='Interactive 3D is unavailable in this browser.';}
});
